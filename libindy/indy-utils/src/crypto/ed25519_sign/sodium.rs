...
pub fn create_key_pair_for_signature(
    ...
    let (public_key, secret_key) = Ed25519Sha512::keypair_from_secret(
        &seed
            .unwrap_or(&Seed::from_slice(&randombytes(SEEDBYTES)).unwrap())
            .as_ref(),
    )
    ...
    Ok((
        PublicKey::from_slice(public_key.as_ref()).unwrap(),
...
pub fn sk_to_curve25519(sk: &SecretKey) -> Result<ed25519_box::SecretKey, IndyError> {
    ...
    let sk = ursa::keys::PrivateKey(sk.as_ref().to_vec());
    let sk_x = Ed25519Sha512::sign_key_to_key_exchange(&sk).unwrap();
    Ok(ed25519_box::SecretKey::from_slice(sk_x.as_ref()).unwrap())
}
...
pub fn vk_to_curve25519(pk: &PublicKey) -> Result<ed25519_box::PublicKey, IndyError> {
    ...
    let pk = ursa::keys::PublicKey(pk.as_ref().to_vec());
    let pk_x = Ed25519Sha512::ver_key_to_key_exchange(&pk).unwrap();
    Ok(ed25519_box::PublicKey::from_slice(pk_x.as_ref()).unwrap())
}
